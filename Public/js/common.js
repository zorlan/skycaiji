...
if(data.status==1){toastr.success(data.info);$_o.find('.verify-img').trigger('click')}else{toastr.error(data.info)}
if(data.js){eval(data.js)}},error:function(data){$_o.find('button[type="submit"]').removeAttr('disabled');toastr.error(data)}});return!1})});function setCookie(c_name,value,expiredays){var exdate=new Date();exdate.setDate(exdate.getDate()+expiredays);document.cookie=c_name+"="+escape(value)+((expiredays==null)?"":";expires="+exdate.toGMTString())+';path='+(window.site_config?(window.site_config.root?window.site_config.root:'/'):'/')}
...
if(document.getElementById('myModal')){$('#myModal').modal('hide');$('#myModal').remove()}
...
modal(title,'<img src="'+window.site_config.pub+'/images/loading.gif" />',options);var ajaxSet={type:'get',url:url,success:function(data){if(htmlIsJson(data)){$('#myModal').modal('hide');ajaxDataMsg(data)}else{modal(title,data,options)}},dataType:'html'};if(options.ajax){ajaxSet=$.extend(ajaxSet,options.ajax)}
...
options.bodyStyle=' style="padding:0;" ';modal(title,'<img src="'+window.site_config.pub+'/images/loading.gif" style="margin:10px;" />',options);var height=parseInt($(window).height());height=height-parseInt($('#myModal .modal-body').offset().top-$(document).scrollTop())*2;$('#myModal .modal-body').css('height',height);$('#myModal .modal-body').html('<iframe id="myModalIframe" src="'+url+'" frameborder="0" width="100%" height="100%" frameborder="0" scrolling="yes"></iframe>');$('#myModal').on('hidden.bs.modal',function(e){$('#myModal iframe').remove()})}
function ajaxDataMsg(data){if(typeof data=='string'){data=eval('('+data+')')}
if(data.status==1){toastr.success(data.info)}else{toastr.error(data.info)}
...
$(obj).attr('src',src)}
function ulink(url,vals){url=url.replace(/^\s*\//,'');if(url.indexOf('/')>-1){var path=url.split('/');if(path.length==2){url='m=admin&c='+path[0]+'&a='+path[1]}else if(path.length==3){url='m='+path[0]+'&c='+path[1]+'&a='+path[2]}}
url=window.site_config.root+'/index.php?'+url.replace('?','&');if(vals){for(var i in vals){url=url.replace(i,encodeURIComponent(vals[i]))}}
return url}

...
$('#myModal .modal-title').html(title);$('#myModal .modal-body').html(body);$('#myModal').modal('show');$('#myModal').on('hidden.bs.modal',function(e){execVarFuncs(options.hidden_func)})}
...
modal(title,'<div class="loading"></div>',options);var ajaxSet={type:'get',url:url,success:function(data){if(dataIsJson(data)){$('#myModal').modal('hide');ajaxDataMsg(data)}else{modal(title,data,options)}},dataType:'html'};if(options.ajax){ajaxSet=$.extend(ajaxSet,options.ajax)}
...
options.bodyStyle=' style="padding:0;" ';modal(title,'<div class="loading" style="margin:10px;"></div>',options);var height=parseInt($(window).height());height=height-parseInt($('#myModal .modal-body').offset().top-$(document).scrollTop())*2;var ifrHtml='<iframe id="myModalIframe" '+(url?(' src="'+url+'"'):'')+' width="100%" height="100%" frameborder="0" scrolling="yes"></iframe>';$('#myModal .modal-body').css('height',height);$('#myModal iframe').remove();$('#myModal .modal-body').html(ifrHtml);$('#myModal iframe').bind('load',function(){$('#myModal').attr('data-iframe-loaded',1);execVarFuncs(options.ifr_loaded_func)});$('#myModal').on('hidden.bs.modal',function(e){$('#myModal iframe').remove();execVarFuncs(options.close_func)});execVarFuncs(options.loaded_func)}

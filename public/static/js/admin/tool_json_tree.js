...
html+='</ul>'}else{html+='<span class="node">'+node+': </span>';if(dataIsJson(list)||dataIsHtml(list)){var eleId='txt_'+generateUUID();list=htmlspecialchars(list);html+='<span class="val"><a href="javascript:;" data-tree-visualize="'+eleId+'">预览</a></span><div class="text"><textarea id="'+eleId+'" rows="3">'+list+'</textarea></div>'}else if(typeof(list)!='object'){html+='<span class="val">'+htmlspecialchars(list)+'</span>'}}
html+='</li>';return html}}

...
abstract class BaseCms extends ReleaseBaseEvent {
	...
	public function init($cmsPath=null,$release=null){
		...
		$className=get_class($this);//类名
		if(preg_match('/([^\/\\\]+)Cms$/i',$className,$cmsApp)){
			$this->cmsApp=$cmsApp[1];//cmsapp
		}
	...
	public function tplBind(){
		if(!empty($this->_params)){
			...
			$this->display(T('Release@Cms:BaseCms'));
		}else{
			$sltCollField=$this->param_option_fields();
			$this->assign('sltCollField',$sltCollField);
			//没有参数调用模板，复杂需求下可使用模板
			$this->display(T('Release@Cms:'.$this->cmsApp.'Cms'));
		}
	}
